...
см. [Экран HUD](/ru/fv-hud/hud-surface). Пак объявляет нужные ему в `entries`; остальные берутся из штатного пака,
файл за файлом.
...
чата, которой страница HUD уже владеет, и сохраняет ванильное текстовое поле. См. [Экран HUD](/ru/fv-hud/hud-surface).
